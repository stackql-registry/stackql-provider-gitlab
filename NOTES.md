...
- Probes for gets with required global-ID arguments render `gid://gitlab/<Model>/1`; a query GitLab rejects before scoring is `UNMEASURED` and passes by proxy when a query on the same node type measured within the limit.
